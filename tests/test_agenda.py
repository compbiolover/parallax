...
def _seed(store: Datastore, layout: dict[str, dict[int, int]]):
    """``{persona: {cluster_id: how many articles}}`` -> documents + a clustering.

    Each persona is given its own source, so persona membership can tell their
    documents apart. Returns the registry that says so.
...
def test_identical_agendas_do_not_diverge():
    store = _store()
    reg = _seed(store, {"self": {0: 20, 1: 20}, "modeled_ce": {0: 20, 1: 20}})
    a = compare_agendas(store, reg, pair())
...
def test_attention_is_measured_in_shares_not_counts():
    ...
    store = _store()
    reg = _seed(store, {"self": {0: 30, 1: 10}, "modeled_ce": {0: 60, 1: 20}})
    shares = attention_shares(store, reg)
    assert shares["self"] == shares["modeled_ce"] == {0: 0.75, 1: 0.25}
    assert compare_agendas(store, reg, pair()).divergence < 1e-9
    store.close()
...
def test_unclustered_documents_are_not_a_story_either_diet_chose():
    ...
    store = _store()
    reg = _seed(store, {"self": {0: 20, -1: 40}, "modeled_ce": {1: 20, -1: 40}})
    a = compare_agendas(store, reg, pair())
    assert a.total_stories == 2
...
def test_thin_coverage_is_flagged_rather_than_hidden():
    store = _store()
    reg = _seed(store, {"self": {0: THIN_ARTICLES}, "modeled_ce": {1: THIN_ARTICLES - 1}})
    assert compare_agendas(store, reg, pair()).thin is True
    store.close()


def test_ample_coverage_is_not_flagged_thin():
    store = _store()
    reg = _seed(store, {"self": {0: THIN_ARTICLES}, "modeled_ce": {1: THIN_ARTICLES}})
    assert compare_agendas(store, reg, pair()).thin is False
    store.close()
...
def test_nothing_clustered_is_absence_not_agreement():
    ...
    assert compare_agendas(store, reg, pair()) is None
    assert attention_shares(store, reg) == {}
    store.close()


def test_one_diet_alone_cannot_be_compared():
    store = _store()
    reg = _seed(store, {"self": {0: 20, 1: 20}})
    assert compare_agendas(store, reg, pair()) is None
    store.close()
...
def test_the_comparison_serializes_flat():
    store = _store()
    reg = _seed(store, {"self": {0: 30, 1: 10}, "modeled_ce": {0: 30}})
    d = compare_agendas(store, reg, pair()).to_dict()
