...
def assign_themes(
    ...
    from_claude = claude_assignments(entries, client, model, effort) if use_claude else {}
    ...
    * a **theme** collects the stories of one subject in one direction.
    ...
    for raw in blindspots:
        ...
        for article in articles.get(spot.cluster_id) or _articles_from_titles(spot):
            assignment = assignments.get(article.doc_id) or taxonomy_theme([article.title])
    ...
    themes.sort(key=lambda t: (t.key == OTHER_KEY, -t.article_count, -t.story_count, t.title))
    return themes
...
def _story(cluster_id: int, rows: list) -> Story:
    ...
    for _, article, _ in rows:
        label = article.outlet or _host(article.url) or _from_source_id(article.source_id)
...
def _articles_from_titles(spot: _Spot) -> list[Article]:
    ...
    """
    return [
        Article(doc_id=f"{spot.cluster_id}:{i}", title=title) for i, title in enumerate(spot.titles)
    ]
